...
% Last Modified by GUIDE v2.5 13-Dec-2020 13:20:39
...
% handles    structure with handles and user data (see GUIDATA)
handles.output = hObject;
[filename,pathname]=uigetfile({'*.bmp;*.tif;*.png;*.jpg','All Image Files'},'Pilih File Gambar');
if ~filename
    errordlg('Pilih File Gambar');
    return;
end
...
colormap gray;
imagesc(handles.img);
set(handles.original_image,'String','Gambar Asli');
...
colormap gray;
imagesc(V);
set(handles.compressed_image,'String','Dekomposisi Level Pertama');
...
if isfield(handles,'threshold')
    ...
    errordlg('Pilih Level dan Masukkan Nilai Threshold untuk kompresi');
    return;
end
else
    thr=0;
...
if isfield(handles,'c')
    ...
    errordlg('Perhitungan DWT pertama');
    return;
end
...
set(handles.compressed_image,'String','Kompresi DWT');
...
function about_Callback(~,~,~)
    msgbox({'Kompresi Gambar DWT dan DCT' '' ...
        'Aplikasi ini dibuat untuk tugas akhir mata kuliah Teknik Kompresi Multimedia Oleh Kelompok 8' ...
        },'About');
...
if isnan(handles.threshold) 
     errordlg('Masukkan Nilai Threshold');
...
if get(handles.threshold_type,'Value')==3
    ...
    warndlg('Masukkan Nilai Level Threshold untuk kompresi');
    return;
end
...
switch type
    case 1
        set(handles.thr_universal_type_description,'String',... 
            'Opsi ini mengembalikan nilai Threshold mendekati 0. Nilai Threshold tipikal adalah median (abs (koefisien)).');
        set(handles.description_panel,'Title','Median');
    case 2
        set(handles.thr_universal_type_description,'String',... 
            'Opsi ini mengembalikan Threshold sedemikian rupa sehingga persentase energi yang dipertahankan dan jumlah nol sama.');
        set(handles.description_panel,'Title','Energy');
    case 3
        set(handles.thr_universal_type_description,'String',... 
            'Opsi ini mengembalikan Threshold yang sama dengan akar kuadrat dari nilai tersebut sehingga persentase energi yang ditahan dan jumlah nol adalah sama.');
        set(handles.description_panel,'Title','Square');
end
...
guidata(hObject, handles);
axes(handles.image_out2);
axis image;
colormap gray;
imagesc(handles.imgrec_dct);
set(handles.compressed_jpeg,'String','Kompresi DCT');
...
set(handles.compressed_jpeg,'String','Kompresi JPEG');
...
% --- Executes during object creation, after setting all properties.
function image_in_CreateFcn(hObject, eventdata, handles)
% hObject    handle to image_in (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called

% Hint: place code in OpeningFcn to populate image_in
